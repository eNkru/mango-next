...
`Howard Ju` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Howard Ju/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Howard Ju/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee Howard Ju` to
...
python3 ./.trellis/scripts/task.py archive 00-join-howard-ju
```
